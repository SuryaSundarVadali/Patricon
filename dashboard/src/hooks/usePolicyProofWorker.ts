...
import type { ContractProofBundle, PolicyProofInput } from "../lib/zk/zkTypes";
import type { ZkWorkerRequest, ZkWorkerResponse } from "../lib/zk/workerTypes";

type Status = "idle" | "running" | "success" | "error";

type WorkerResult = {
  proof: ContractProofBundle<14>;
  elapsedMs: number;
};
...
export function usePolicyProofWorker() {
  ...
  const generate = useCallback((input: PolicyProofInput): Promise<WorkerResult> => {
    ...
    return new Promise<WorkerResult>((resolve, reject) => {
      ...
      const handleMessage = (event: MessageEvent<ZkWorkerResponse>) => {
        ...
        const parsed = deserializeFromWorker(event.data.payload) as ContractProofBundle<14>;
